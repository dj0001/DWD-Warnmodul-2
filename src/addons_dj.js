...
var auto=300  //update-interval in s (alle 5 min; 2e6 no-updates)  //edit here
  ...
  auto=Math.max(300,auto); auto*=1000; if((navigator.connection||{}).type=="cellular") auto*=2  //reduce traffic
  ...
  var t0=Date.now(); setInterval(function(){update()}, 2*auto)  //add update
  ...
  function update() {if(Date.now()-t0>=2*auto && !document.hidden) {t0=Date.now(); warnlayer.setUrl("https://maps.dwd.de/geoproxy_warnungen/service/?" + Math.random());
...
var dt=0; warnlayer._marker.on('move', function(e){ var data=warnlayer._data
     ...
     {if(qs!=="-0"||qs==="-0"&&document.hidden) showNotification(data.features.length); dt++}} else dt=0;  //dt reduces noti //?-0 notification, only hidden document
})
        ...
        fetch("https://nominatim.openstreetmap.org/search?q=" + q + "&format=json&limit=1")
          .then(function(response) {
            response.json().then(function(data) {var e = [data[0].lat,data[0].lon];
              karte.setView(e,6,{animate:false});
...
var geo=L.control.watermark({position: 'bottomright'}).addTo(karte)  //add geocode
  ...
  if(isNaN(qs) &&qs.match(/^ort=/)) {qs=qs.slice(4); if(qs!=0&& qs.match(/^[\d,-\.]+$/)) {karte.setView(qs.split(","),6,{animate:false}); warnlayer.getFeatureInfoJsonp({latlng: qs.split(",")})}  //?ort=48.37,10.9
